  ...
  methods: {
    openDialogItem(index) {
      ...
      if(index < 0) {
        ...
        this.formItem.title = this.menuItems[index].title
      }
    },
    saveItem() {
      if(this.selectedItemIndex < 0){
        this.menuItems.push(this.formItem)
      }else {
        this.menuItems[this.selectedItemIndex] = this.formItem
      }
      this.save()
    },
    async save () {
      try {
        await this.$firebase.database().ref().child('site').update({ menu: this.menuItems })
      } finally {
